...
(function (global) {
  ...
  VoxelWorld.prototype._tdmNearSpawn = function (x, z, radius) {
   ...
   */
  VoxelWorld.prototype._buildTdmDistricts = function (heightCap) {
    const cap = Math.max(8, heightCap | 0);
    const spacing = 24 + this._randInt(0, 4);
    const bounds =
      this._tdmArenaBounds ||
    ...
    for (let bx = bx0; bx < bx1 - 16; bx += spacing) {
      for (let bz = bz0; bz < bz1 - 16; bz += spacing) {
        const px = bx + 8 + this._randInt(-4, 5);
        const pz = bz + 8 + this._randInt(-4, 5);
        if (px < bx0 - 2 || pz < bz0 - 2 || px >= bx1 - 2 || pz >= bz1 - 2) continue;

        const footprint = 9 + this._randInt(0, 7);
        const cx = px + footprint * 0.5;
        const cz = pz + footprint * 0.5;
        if (this._tdmNearSpawn(cx, cz, TDM_CLEAR_R)) continue;
        ...
        if (this._noise(pz * 0.17, px * 0.19) < 0.1) continue; // occasional empty lot
  ...
  VoxelWorld.prototype._rebuildChunk = function (cx, cz) {
    ...
    const mesh = new THREE.Mesh(geo, this._chunkMat);
  ...
  VoxelWorld.prototype._markChunkDirty = function (cx, cz) {
  ...
  VoxelWorld.prototype.flushRebuilds = function (maxPerFrame, preferX, preferZ) {
    ...
    if (!this._dirtyChunks.size) return;
  ...
  VoxelWorld.prototype.overlapsSolid = function (box) {
    ...
    for (let x = minX; x <= maxX; x++) {
      for (let y = minY; y <= maxY; y++) {
        for (let z = minZ; z <= maxZ; z++) {
          if (this._isSolid(x, y, z)) return true;
        }
      }
    }
  ...
  /** Highest walkable surface (voxels + stair tops) at xz */
  VoxelWorld.prototype.getWalkHeight = function (x, z) {
    const fx = Math.floor(x);
    const fz = Math.floor(z);
    let y = this.height - 1;
    // Coarse skip through empty sky, then fine scan
    while (y > 4 && !this._isSolid(fx, y, fz)) y -= 4;
    y = Math.min(this.height - 1, y + 4);
    while (y > 0 && !this._isSolid(fx, y, fz)) y--;
    let best = y + 1;
    const props = this.props;
    for (let i = 0; i < props.length; i++) {
      const p = props[i];
  ...
  VoxelWorld.prototype.dirtyRect = function (x0, z0, x1, z1) {
    const cs = this.chunkSize;
